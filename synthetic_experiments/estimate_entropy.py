...
def get_dataloader(data_generator, n_batchsize, n_iterations, cubic=False):
    ...
    # sampling from the generator distribution
    training_dataset_np = data_generator.rvs(size=(n_batchsize * n_iterations,))
    
    if cubic:   # raising the data to the power three for the cubic distribution
...
def main(args):
    ...
    # making the settings string for the logger  
    settings = f"""
    Settings:
    ...
    n_batchsize = args.batchsize                # batch size
    n_iterations = args.iterations              # number of iterations 
    n_epochs = args.epochs                      # number of epochs 
    n_samples_kernel = args.kernel_samples      

    log_dir = args.logdir                       # the path of the logging directory 
    if log_dir is None:
        log_dir = 'results'
    ...
    d = args.dimension
    if args.pdf == 'gaussian':
        cov = np.eye(d)                         # the identity covariance matrix 
        data_generator = multivariate_normal(cov=cov)       # the multivariate random variable
        data_generator.entropy = lambda: np.prod(slogdet(2*np.pi*np.exp(1)*cov))/2  # calculating the entropy of the multivariate
    else:
        # making the data random generator according to the pdf
    ...
    dataloader = get_dataloader(data_generator,
                                ...
                                cubic=args.cubic)
    # making the intialization samples for initializing the network 
    ...
    G_adaptive = MargKernel(args_adaptive,
                            args.dimension,
                            None,
                            init_samples= cu(torch.tensor(samples)),
                            )
    cu(G_adaptive)
    ...
    try:
        for i_epoch in range(n_epochs):
            ...
            for i in tqdm(range(len(dataloader))): #len dataloader = number of iterations usually
                i += i_epoch * n_iterations
                if (i+1) % iter_delta == 0 and args.pdf == 'gaussian':
                    cov = cov * args.rho            # multiplying the covariance matrix with constant at each step
                    ...
                    if args.cubic:
                        if args.pdf == 'gaussian':
                            true_entropy += 3*args.dimension*np.log(args.rho)/2 # modifying the entropy because of Cov change
                        else:
                            assert False, 'Cubic only supported for gaussian pdf'
                    else:
                        true_entropy += args.dimension * np.log(args.rho) / 2   # modifying the entropy  because of Cov change
                x = next(data_iter)
                for o in opts:
                    o.zero_grad()       # reset the gradients of each parameter
                x = x[0]
                x = cu(x)
                for model, opt, name in zip(models, opts, model_names):
                    loss_adaptive = model(x)
                    loss_adaptive.backward()
                    loss_np = loss_adaptive.detach().cpu().numpy()      # because true entropy exists in CPU
    ...
    # Final estimate after finishing the training 
    dataloader = get_dataloader(data_generator,
                                n_batchsize=n_batchsize,
                                n_iterations=n_iterations,
                                cubic=args.cubic)
