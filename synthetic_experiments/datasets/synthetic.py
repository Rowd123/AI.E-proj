...
class DataGeneratorBase:
    ...
    def rvs(self, size=(1,)):               # sampling from the random variable
        raise NotImplementedError()

    def entropy(self):                      # calculating the differntial entropy 
        raise NotImplementedError()

    def get_Lipschitz(self, L_base):        # calculating the Lipschitz constant
    ...
    def plot(self):                         # plot the pdf of the distribution 
        raise NotImplementedError('Plot of base_distribution not known')

    def pdf(self, x):                       # calculating the pdf 
        xs, ys = self.plot()
        return np.interp(x=x, xp=xs, fp=ys)

    def logpdf(self, x):                    # calculating the log of the pdf
...
class DataGeneratorPiecewise(DataGeneratorBase):
    ...
    def __init__(self,
        ...
        self.base_distribution = base_distribution      # the base multivariable distribution
        self.interval = interval                        # the interval of the pdf 

        weights = np.array(weights)                     # the weight of each piecewise distribution
        starts = np.array(starts)                       # the start of each piecewise distribution
        scales = np.array(scales)                       # the length of each piecewise distribution

        assert weights.shape == starts.shape == scales.shape
        n_mixture = weights.shape[0]                    # number of pieces 

        self.starts = starts                            
        self.weights = weights
        self.scales = scales
        
        # sampling from according to the weights to determine the piece to sample from 
        self.selection_dist = stats.rv_discrete(b=n_mixture,
                                                values=(range(n_mixture), self.weights))
        self._check_no_overlap()

    def rvs(self, size=(1,)):
        # the random choice of the pieces 
        selection = self.selection_dist.rvs(size=size)
        
        # the start of each piece 
        offsets = self.starts[selection]
        
        # the lengths of each piece 
        scales = self.scales[selection]
        
        # making the distribution
        distr = self.base_distribution(scale=scales)
        
        # sampling from the made distribution
    ...
    def entropy(self):
        ...
        # the entropy is the sum of the entorpies of the selection and the continuous variables 
    ...
    def _check_no_overlap(self):
        # Check if the piecewise distribution have overlapping intervals
...
class TriangleGenerator(DataGeneratorPiecewise):
    def __init__(self, *args, **kwargs):
        # base distribution with symmetric triangles 
        base_distribution = partial(stats.triang, c=.5)
        super().__init__(*args, base_distribution=base_distribution, **kwargs)
