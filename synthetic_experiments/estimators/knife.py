...
class MargKernel(nn.Module):
    def __init__(self, args, zc_dim, zd_dim, init_samples=None):
        ...
        self.K = args.marg_modes if self.optimize_mu else args.batch_size   # the number of kernels
        self.d = zc_dim             # the dimension of the sample
        self.use_tanh = args.use_tanh   # using tanh for the variance 
        self.init_std = args.init_std   # the initialization standard variation 
        super().__init__()

        self.logC = torch.tensor([-self.d / 2 * np.log(2 * np.pi)])     # -d*log(2*pi)/2

        if init_samples is None: # if no initalization samples is given make them 
        ...
        if args.cov_diagonal == 'var': # making the variance 'the diagonal matrix'
        ...
        if args.cov_off_diagonal == 'var': # making the variance 'the off diagonal elements'
        ...
        weigh = torch.ones((1, self.K))     # making weighs for each sample 
        if args.average == 'var':              
...
class CondKernel(nn.Module):
    ...
    def logpdf(self, z_c, z_d): 
        ...
        w = torch.log_softmax(self.weight(z_c), dim=-1)  
        mu = self.mu(z_c)
